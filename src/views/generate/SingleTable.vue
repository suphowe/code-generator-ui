        ...
        <el-form ref="optionForm" class="optionForm" :inline="true" label-width="70px"
          ...
          <span>
            <el-form-item label="包名">
              <el-input v-model="generateModel.basePackage" placeholder="如：com.vuebg">
...
export default {
  ...
  methods: {
    ...
    tableSelectionChange(selections) {
      ...
      axios.post(this.baseUrl + "/getGenerateModel", params).then(res => {
        ...
        if (res.code == 200) {
          ...
          this.generateModel.outPutFolderPath = this.global.outPutFolderPath
    ...
    // 选择代码输出目录
    chooseBasePackage() {
      this.generateModel.basePackage = this.global.basePackage
    },
    // 选择代码输出目录
    chooseOutputFolder() {
      this.generateModel.outPutFolderPath = this.global.savePath
    },
    // 生成代码
    generateCode() {
